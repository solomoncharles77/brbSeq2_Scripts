...
samFiles <- readLines("/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/brbSeq2_Scripts/allSam.txt")

clean <- featureCounts(files = paste0("/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/mappedReads/", samFiles),
                       annot.ext = "/scratch/vasccell/cs806/exprPhenoData/Homo_sapiens.GRCh38.100.gtf",
                       isGTFAnnotationFile = TRUE,
                       GTF.featureType = "exon",
                       GTF.attrType = "gene_id",
                       strandSpecific = 1, isPairedEnd = F,
                       nthreads = 8)

count <- as.data.frame(clean$counts)
colnames(count) <- gsub(".Aligned.out.sam", "", colnames(count))
# export featureCount results
write.csv(count, file = "/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/readCount/rawReadCount.csv")
saveRDS(clean, file = "/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/readCount/readFeatures.rds")

# export individual count files
for (i in seq_along(colnames(clean$counts))) {
  df <- cbind(clean$annotation, clean$counts[, i])
  colnames(df)[7] <- "count"
  write.table(df, file = paste0("/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/readCount/",
...
write.table(tg, "/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/brbSeq2_Scripts/target.txt", quote = F, sep="\t")
...
# write.csv(norm, file = "/lustre/alice3/scratch/vasccell/cs806/brbSeq/brbSeq2/readCount/normReadVals.csv")
